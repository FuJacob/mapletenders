      ...
      <div className="max-w-lg relative z-10">
        ...
        <div className="space-y-6 mb-10">
          {benefits.map((benefit, index) => (
            <div
              key={index}
              className="group flex items-start gap-4 p-4 rounded-xl hover:bg-background/50 transition-all duration-300 hover:shadow-sm"
            >
              <div
                className={`flex-shrink-0 w-14 h-14 ${benefit.iconBg} rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-sm`}
              >
                {benefit.icon}
              </div>
        ...
        <div className="mt-8 p-6 bg-gradient-to-r from-primary/5 to-accent/5 border border-primary/10 rounded-2xl">
          <div className="text-text-muted text-sm italic mb-2">
            "Mapletenders helped us win our first $500K government contract
