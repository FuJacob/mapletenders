...
export class SearchService {
  ...
  async searchTenders(
    ...
    try {
      ...
      if (filters.timeline?.publishedAfter) {
        query = query.gte(
          "published_date",
          filters.timeline.publishedAfter.toISOString()
        );
      }
      ...
      if (filters.timeline?.closingBefore) {
        query = query.lte(
          "closing_date",
          filters.timeline.closingBefore.toISOString()
        );
      }
      ...
      if (filters.categories?.procurementTypes?.length) {
        query = query.in(
          "procurement_type",
          filters.categories.procurementTypes
        );
      }
      ...
      if (filters.advanced?.keywords?.length) {
        const keywordFilters = filters.advanced.keywords
          .map(
            (keyword) =>
  ...
  async getSearchAggregations(filters: SearchFilters): Promise<any> {
    try {
      ...
      const locations = this.aggregateField(
        locationData,
        "contracting_entity_province"
      );
    ...
    try {
      ...
      const queries =
        data
          ...
          .filter(
            (q: any) => q && q.toLowerCase().includes(query.toLowerCase())
          )
          .filter(
            (q: any, index: number, arr: any[]) => arr.indexOf(q) === index
          ) // Remove duplicates
          .slice(0, 10) || [];
  ...
  async trackSearchPerformance(
    ...
    try {
      await supabase.from("search_performance").insert([
        {
          search_id: searchId,
          user_id: userId,
