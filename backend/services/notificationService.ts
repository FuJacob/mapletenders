...
export class NotificationService {
   ...
   */
  async createDeadlineAlert(alert: Omit<DeadlineAlert, 'id'>): Promise<DeadlineAlert> {
    try {
      const { data, error } = await supabase
  ...
  async sendPendingNotifications(): Promise<void> {
    try {
      ...
      for (const notification of notifications || []) {
        await this.sendNotification(this.mapDatabaseToNotification(notification));
      }
    } catch (error) {
   ...
   */
  private mapDatabaseToNotificationPreference(data: any): NotificationPreference {
    return {
      id: data.id,
      userId: data.user_id,
      type: data.type,
